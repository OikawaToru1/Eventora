  ...
  return (
    <div className="w-[84vw] h-[350px] mx-auto mt-3 lg:w-[88vw] ">
      <div className='relative w-[100%] h-[100%] rounded-[16px] shadow-lg shadow-[#5a5a5a]' style={background}>
        <div className="absolute top-12 left-[4%] text-[36px] text-white lg:w-[600px] lg:text-[50px] leading-[50px] lg:leading-[58px]">
          <span className="capitalize font-medium ">{CarouselTitle}</span>
          <h3 className="text-[23px] lg:text-[34px] pt-3">{CarouselDescrip}</h3>
        </div>
        ...
        <div className="flex gap-[20px] absolute bottom-0 left-[40%] sm:left-[45%] lg:left-[50%] ">
          {slides.map((image,index) => (
            <div className="text-[54px] font-medium text-white cursor-pointer hover:text-[#393939]" key={index} onClick={() =>  goToSlide(index)}>.</div>
          ))}
        </div>
      </div>  
    </div>
